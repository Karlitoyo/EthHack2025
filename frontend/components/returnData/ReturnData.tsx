...
export default function FamilySearch() {
  ...
  const fetchFamilyData = async () => {
    ...
    try {
      ...
      // Catches network errors or issues with res.json() if res was ok
      ...
      if (errorToShow) {
        ...
        if (errorModalRef.current) {
          ...
          console.warn(
            "Error modal ref is not available. Ensure 'errorModalRef' is defined and passed to a <dialog> element."
          );
        }
      }
    }
  ...
  return (
    <div className='container w-[750px] mx-auto p-4 sm:p-6 md:p-8'>
      {/* Error Modal Dialog */}
      <dialog
        id='error_modal'
        className='modal'
      ...
      <div className='card w-full max-w-7xl bg-base-100 shadow-xl mx-auto'>
        <div className='card-body p-6 sm:p-8'>
          ...
          {error && (
            <div
              role='alert'
              className='alert alert-error my-6 animate-fadeIn'
            >
              <svg
          ...
          {data && (
            <div className='mt-6 bg-base-200 shadow-xl rounded-lg p-4 sm:p-6'>
              ...
              <div className='mb-6 pb-4 border-b border-base-300'>
                ...
                <p>
                  <span className='font-medium'>Name:</span> {data.targetRelation.firstName}{' '}
                  {data.targetRelation.lastName}
                </p>
                <p>
                  <span className='font-medium'>ID:</span> {data.targetRelation.id}
                </p>
              </div>
                  ...
                  <ul className='list-disc list-inside pl-4'>
                    {data.lineagePath.map((ancestor, index) => (
                      <li
                        ...
                        className='mb-1'
                      >
                        {ancestor.name} {ancestor.roleInFamily ? `(${ancestor.roleInFamily})` : ''} (Family ID:{' '}
                        {ancestor.familyId || 'N/A'})
                      </li>
                    ))}
                  </ul>
                </div>
              ...
              {!data.lineagePath?.length && !data.siblings?.length && (
                <p className='text-center text-info'>
                  No lineage or sibling information available for this individual.
                </p>
              )}
            </div>
          )}
          ...
          {!loading && !data && !error && (
            <div
              className='text-center py-10 animate-fadeIn'
              style={{ animationDelay: '0.1s' }}
            >
              <svg
                xmlns='http://www.w3.org/2000/svg'
                className='h-16 w-16 mx-auto text-base-content opacity-30 mb-4'
                fill='none'
                viewBox='0 0 24 24'
                stroke='currentColor'
                strokeWidth='1'
              >
                <path
                  strokeLinecap='round'
                  strokeLinejoin='round'
                  d='M10 21h7a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v11m0 5l4.879-4.879m0 0a3 3 0 104.242-4.242 3 3 0 00-4.242 4.242zM10 17h4'
                />
              </svg>
              ...
              <p className='text-sm text-base-content opacity-50'>
                Family lineage and sibling information will be displayed here.
              </p>
            </div>
          )}
        </div>
